...
``` json
{
  ...
  "config-overrides-path": "node_modules/@skiofox/react-app-rewire-micro-frontends",
  "devDependencies": {
    "@skiofox/react-app-rewire-micro-frontends": "0.0.2",
...
我们把扩展的 `webpack 配置文件` 封装到一个npm 包里去，并通过 `config-overrides-path` 指向到文件所在位置。
